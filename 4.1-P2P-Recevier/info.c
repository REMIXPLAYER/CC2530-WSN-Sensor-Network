...
*********************************************************************************************/
void lcd_dis(void){
  for(unsigned char i = 0;i<2;i++){                              //发送TYPE，循环2次
    printf("{TYPE=02101}");                                      //调用实验
    halWait(250);
    halWait(250);
  }
  ...
  CC2530_MAC[28]='}';
   for(unsigned char i = 0;i<2;i++){                            //发送MAC，循环2次
   printf(CC2530_MAC);  
   halWait(250);
   halWait(250);
  }
